...
from graphsage.utils2 import load_cora,load_wikics,construct_agg, load_ppi, F1_score, custom_load_pubmed, get_average_lsh_added, load_ogbn_protein, load_ogbn_arxiv, create_graph_from_representation
import pickle
import json
...
class SupervisedGraphSage(nn.Module):
    ...
    def __init__(self, num_classes, enc):
        ...
        self.weight = nn.Parameter(torch.empty(num_classes, enc.embed_dim, device = self.device).float())
...
def evaluate(model,test,out_dir,labels,epoch,classification,cuda):
    results={}
    #test_output = model.forward(test)
    test_batch_size = 64
    ind = 0
    for batch in range(0,len(test),test_batch_size):
        test_batch_nodes = test[batch:batch+test_batch_size]
        test_batch_output = model.forward(test_batch_nodes)
        if ind == 0:
    ...
    if classification == 'normal':
        if not cuda:
            ...
            test_f1 = f1_score(labels[test], test_output.data.cpu().numpy().argmax(axis=1), average="micro")
            test_f1_macro = f1_score(labels[test], test_output.data.cpu().numpy().argmax(axis=1), average="macro")
    elif classification == 'multi_label':
        if not cuda:
            test_f1 = F1_score(labels[test], torch.sigmoid(test_output).data.numpy())
        else:
            test_f1 = F1_score(labels[test], torch.sigmoid(test_output).data.cpu().numpy())    
        test_f1_macro = 0.70
    results['epoch']=epoch
...
### Helper function to evaluate OGBN-Arxiv
def test_ogbn(model, true_labels, split_idx, evaluator, out_dir, epoch,num_nodes):
    model.eval()
    results = {}
    batch_size = 16
    ind=0
    ...
    for batch in range(0,num_nodes,batch_size):
        ...
        batch_output = model.forward(batch_nodes)
        if ind == 0:
            output = batch_output
        else:
    ...
    train_acc = evaluator.eval({
        'y_true': true_labels[split_idx['train']].reshape(-1,1),
        'y_pred': y_pred[split_idx['train']].reshape(-1,1),
    })['acc']
    valid_acc = evaluator.eval({
        'y_true': true_labels[split_idx['valid']].reshape(-1,1),
        'y_pred': y_pred[split_idx['valid']].reshape(-1,1),
    })['acc']
    test_acc = evaluator.eval({
        'y_true': true_labels[split_idx['test']].reshape(-1,1),
        'y_pred': y_pred[split_idx['test']].reshape(-1,1),
    })['acc']
...
def run_general(name,outdir,rw=False,neighbours1=20,neighbours2=20,epochs=100,attention='normal',aggregator='mean',n_layers=1,random_iter=1,lr=0.01,includenodefeats="no", type_of_walk = 'default', p=1,q=1,num_walks=10,walk_length=10,teleport=0.2, teleport_khop=False, dfactor=2, save_predictions = False, n_vectors = 16, search_radius = 2, atleast = False, num_lsh_neigbours=10, n_lsh_neighbours_sample = None, augment_khop=False, includeNeighbourhood = False, gcn = False, load_embeds=False, planetoid = False, embed_dim = 128, dropout = 0, batch_size = 256, pathtorepfile=None,threshold=0.8):
    ...
    for k in range(random_iter):
        ...
        if name == 'ogbn_proteins':
            data_dic = load_ogbn_protein(lsh_helper, random_walk=rw, teleport=teleport, teleport_khop=teleport_khop, dfactor=dfactor, augment_khop=augment_khop, load_embeds= load_embeds)
            classification='multi_label'
        if name == 'ogbn_arxiv':
            data_dic = load_ogbn_arxiv(lsh_helper, random_walk=rw, teleport=teleport, teleport_khop=teleport_khop, dfactor=dfactor, augment_khop=augment_khop, load_embeds= load_embeds)
            evaluator = Evaluator(name='ogbn-arxiv')
        if name == 'image':
            if pathtorepfile == None:
                data_dic = create_graph_from_representation(threshold =threshold)
            else:
                data_dic = create_graph_from_representation(path_file = pathtorepfile,  threshold=threshold)
        if augment_khop:
        ...
        features = nn.Embedding(data_dic['feat_data'].shape[0],data_dic['feat_data'].shape[1], device=device)
        features.weight = nn.Parameter(torch.tensor(data_dic['feat_data'], device=device).float(), requires_grad=False)
    ...
    ## Training Loop
        ...
        if aggregator == 'mean':
            ...
            #agg1 = PureMeanAggregator(features, cuda=True, gcn=gcn)
            enc1 = Encoder(features, num_feats, embed_dim, data_dic['adj_lists'], agg1, gcn=gcn, cuda=cuda,
            ...
            if n_layers > 1:
                ...
                #agg2 = PureMeanAggregator(lambda nodes: enc1(nodes).t(), cuda=cuda, gcn=gcn)
                enc2 = Encoder(lambda nodes: enc1(nodes).t(), enc1.embed_dim, embed_dim, data_dic['adj_lists'], agg2,
                               base_model=enc1, gcn=gcn, cuda=cuda, feature_labels=data_dic['cluster_labels'],
        ...
        ## New LSH Formulation
            ...
            agg1 = PureMeanAggregator(features, cuda=cuda, gcn = gcn)
            lsh_agg1 = PureMeanAggregator(features, cuda=cuda, gcn=gcn)
            enc1= LSHNeighboursEncoder(features=features, feature_dim=num_feats, embed_dim=embed_dim, adj_lists=data_dic['adj_lists'], aggregator=agg1,lsh_aggregator=lsh_agg1, num_sample=n1, gcn=gcn, cuda=cuda, lsh_neighbours = data_dic['lsh_neighbour_list'], n_lsh_neighbours = n_lsh_neighbours, lsh_augment=augment_khop)
            if n_layers > 1:
                agg2 = PureMeanAggregator(lambda nodes: enc1(nodes).t(), cuda=cuda, gcn=gcn)
                lsh_agg2 = PureMeanAggregator(lambda nodes: enc1(nodes).t(), cuda=cuda, gcn=gcn)
                enc2 = LSHNeighboursEncoder(lambda nodes: enc1(nodes).t(), feature_dim=enc1.embed_dim, embed_dim=embed_dim,
                                            ...
                                            num_sample=n2, gcn=gcn, cuda=cuda,
        ...
        if data_dic['train_mask'] != []:
            ...
            split_idx = {'train':train,'valid':val,'test':test}
        ...
        for epoch in range(epochs):
            ...
            for batch in range(0,len(train),batch_size):
                ...
                optimizer.zero_grad()
                loss = graphsage.loss(batch_nodes,Variable(torch.tensor(labels[np.array(batch_nodes)],device=device).long()),classification)
                loss.backward()
                optimizer.step()
                if cuda:
                  training_loss_epoch.append(loss.data.cpu().numpy())
                else:
            ...
            with torch.no_grad():
                graphsage.eval()
                val_loss = graphsage.loss(val_nodes, Variable(torch.tensor(labels[np.array(val_nodes)],device=device).long()),classification)
            ...
            if epoch%5 == 0:
                ...
                print(epoch, " training loss: " +str(sum(training_loss_epoch)/len(training_loss_epoch)) + "  Validation Loss: "+str(val_loss.data)+ " Time: "+str(epoch_time))
            ## Every 20 epochs or the final epoch, write the test loss to file.
            if epoch%5 == 0 or epoch == epochs-1:
                if name == 'ogbn_arxiv':
                    test_ogbn(model = graphsage, out_dir=out_dir, evaluator =evaluator, true_labels = labels, epoch =epoch, split_idx = split_idx, num_nodes = num_nodes)
                else:
                    evaluate(graphsage,test, out_dir,labels,epoch,classification,cuda)
        
        ##print average time per epoch and total time
    ...
    if save_predictions:
        ...
        for batch in range(0,len(test),test_batch_size):
            test_batch_nodes = test[batch:batch+test_batch_size]
            test_batch_output = graphsage.forward(test_batch_nodes)
            if ind == 0:
                test_output = test_batch_output
            else:
                test_output = torch.cat((test_output,test_batch_output),0)
            ind += 1
        if cuda:
            predicted_scores=test_output.data.cpu().numpy().argmax(axis=1)
            #embeds = graphsage.returnembedding(test).detach().t().cpu().numpy()
        else:
        ...
        for batch in range(0, len(nodes), batch_size):
            batch_nodes = nodes[batch:batch + batch_size]
            if cuda:
                batch_embeds = graphsage.returnembedding(np.array(batch_nodes)).detach().t().cpu().numpy()
            else:
