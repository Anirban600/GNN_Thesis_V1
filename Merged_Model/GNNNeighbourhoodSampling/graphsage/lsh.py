...
def get_nearest_neighbors(X_tfidf, query_vector, model, max_search_radius=3, max_nn=20):
    table = model['table']
    ...
    for search_radius in range(max_search_radius + 1):
        temp = search_nearby_bins(bin_index_bits, table, search_radius, set())
        if(len(temp) < max_nn):
            candidate_set.update(temp)
            max_nn -= len(temp)
        else:
            candidate_set.update(set(list(random.sample(list(temp), max_nn))))
            break
