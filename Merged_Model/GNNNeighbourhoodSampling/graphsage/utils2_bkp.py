...
def load_cora(lsh_helper, random_walk=False, root_folder='/content/drive/MyDrive/HonsProject/datasets/cora',
    ...
    if load_embeds:
        if not planetoid:
            feat_data = np.load('/content/drive/MyDrive/HonsProject/res/cora/gswithembeds/allnodeembeddings.npy')
            print('loading gs embeddings as node features ', feat_data.shape)
        else:
            with open(r'/content/drive/MyDrive/GAT/GAT_Embeddings/Emb_cora_khop-False_add-False-linear.pickle', 'rb') as f:
                emb = pickle.load(f)
            feat_data = emb['embeddings'].detach().cpu().numpy()
            print('loading GAT embeddings as node features ', feat_data.shape)
    ...
    if augment_khop:
        for key, value in adj_lists.items():
            ...
            if np.all((feat_data[node] == 0)):
                ...
                lsh_neighbourlist_dic[node] = lsh_cand_dic[node]
    if teleport_khop:
...
def return_lsh_candidates(features, n_vectors=16, search_radius=3, num_lsh_neighbours=10, atleast=False,
    ...
    for item_id in tqdm(range(features_copy.shape[0])):
        ...
        nearest_neighbors = get_nearest_neighbors(features_copy, query_vector.reshape(1, -1), model,
                                                  max_search_radius=search_radius, max_nn=num_lsh_neighbours)
        count = 0
        if atleast:
            if len(nearest_neighbors) < num_lsh_neighbours:
                radius = search_radius + 1
...
def load_wikics(lsh_helper, random_walk=False, type_walk='default', p=1, q=1, num_walks=10, walk_length=10,
    ...
    ##Root Folder
    root_folder = '/content/drive/MyDrive/HonsProject/datasets/Dataset-WikiCS'
    ...
    if load_embeds:
        with open("/content/drive/MyDrive/GAT/GAT_Embeddings/Emb_wikics_lsh_add-False_linear.pickle", "rb") as f:
            x = pickle.load(f)
        feat_data = x.detach().cpu().numpy()
        print('loading GAT embeddings as node features ',feat_data.shape)
    ...
    if not random_walk:
        ...
        if teleport_khop or augment_khop:
            ...
            et = time.time()
            print('done...time taken (in seconds) = ', et-st)
        else:
            lsh_cand_dic = {}
        if augment_khop:
            for i, row in node_df.iterrows():
...
def custom_load_pubmed(lsh_helper, random_walk=False, root_folder='/home/thummala/graphsage-pytorch/datasets/pubmed-data',
    ...
    if load_embeds:
        try:
            with open("/content/drive/MyDrive/GAT/GAT_Embeddings/Emb_pubmed_khop-False_add-False-linear.pickle", 'rb') as f:
                x = pickle.load(f)
            feat_data = x['embeddings'].detach().cpu().numpy()
            print('loading GAT embeddings as node features ', feat_data.shape)
        except:
...
## Loader for ppi
def load_ppi(lsh_helper, random_walk=False, root_folder='/content/drive/MyDrive/HonsProject/datasets/Dataset-PPI/ppi',
    ...
    if load_embeds:
        feat_data = np.load('/content/drive/MyDrive/HonsProject/res/ppi/gcn_50_saveembeds/allnodeembeddings.npy')
        print('loading gcn embeddings as node features ', feat_data.shape)
    ...
    if not random_walk:
        ...
        if teleport_khop or augment_khop:
            ...
            et = time.time()
            print('done...time taken is, ',et-st)
        else:
            lsh_cand_dic = {}
        for i, row in node_df.iterrows():
            node = int(row['nodes'])
...
def edge_index2adj_list(edge_index,directional=False):
    adj_list = {}
    nodes = np.unique(edge_index[0])
    for node in nodes:
        adj_list[node] = set()
    if directional:
        nodes2 = np.unique(edge_index[1])
        for node in nodes2:
            adj_list[node]=set()
    for edge in np.transpose(edge_index):
        adj_list[edge[0]].add(edge[1])
        if directional:
            adj_list[edge[1]].add(edge[0])
    return adj_list
def compute_node_feats_from_edge(adj_list,edge_feat):
    node_feat=np.zeros((len(adj_list),edge_feat.shape[1]))
    for node, neighs in adj_list.items():
        mean_array = np.mean(edge_feat[list(neighs)],axis=0)
        node_feat[node] = mean_array
    return np.array(node_feat)
def process_ogbn_protein(graph):
    edge_feat = graph['edge_feat']
    node_feat = graph['node_feat']
    edge_index = graph['edge_index']
    adj_list = edge_index2adj_list(edge_index)
    feat_data = compute_node_feats_from_edge(adj_list,edge_feat)
    return adj_list,feat_data
def load_ogbn_protein(lsh_helper,random_walk = False, root_folder='/content/drive/MyDrive/HonsProject/datasets',teleport_khop=False, augment_khop=False, dfactor=2, use_centroid=False, teleport=0.2, load_embeds = False):
    dataset = NodePropPredDataset(name = 'ogbn-proteins', root = root_folder)
    split_idx = dataset.get_idx_split()
    train_idx, val_idx, test_idx = split_idx["train"], split_idx["valid"], split_idx["test"]
    graph, labels = dataset[0] # graph: library-agnostic graph object
    adj_lists, feat_data = process_ogbn_protein(graph)
    ...
    if load_embeds:
        try:
            feat_data = np.load('/content/drive/MyDrive/HonsProject/res/ppi/gswithembeds/allnodeembeddings.npy')
            print('loading gnn embeddings as node features ', feat_data.shape)
        except:
            print('failed to load emebeddings')
    if teleport_khop or augment_khop:
        print('creating lsh')
        lsh_cand_dic = return_lsh_candidates(np.array(feat_data), n_vectors=lsh_helper['n_vectors'],
                                             num_lsh_neighbours=lsh_helper['num_lsh_neighbours'],
    ...
    data_loader_dic = {'feat_data': feat_data, 'labels': labels, 'adj_lists': adj_lists, 'train_mask': train_mask,
                       ...
                       'cluster_labels': [], 'freq': {}, 'dist_in_graph': {},
                       'centralityev': [], 'centralitybtw': [], 'centralityh': [],
                       'centralityd': [], 'lsh_neighbour_list': lsh_neighbourlist_dic}
    return data_loader_dic
def process_ogbn_arxiv(graph):
    node_feat = graph['node_feat']
    edge_index = graph['edge_index']
    adj_list = edge_index2adj_list(edge_index, directional=True)
    return adj_list,node_feat
def load_ogbn_arxiv(lsh_helper,random_walk = False, root_folder='/content/drive/MyDrive/HonsProject/datasets',teleport_khop=False,
                    augment_khop=False, dfactor=2, use_centroid=False, teleport=0.2, load_embeds = False, edge_index_load = False):
    dataset = NodePropPredDataset(name = 'ogbn-arxiv', root = root_folder)
    split_idx = dataset.get_idx_split()
    train_idx, val_idx, test_idx = split_idx["train"], split_idx["valid"], split_idx["test"]
    graph, labels = dataset[0] # graph: library-agnostic graph object
    adj_lists, feat_data = process_ogbn_arxiv(graph)
    num_nodes = len(adj_lists)
    train_mask = []
    test_mask = []
    val_mask = []
    ...
    if load_embeds:
        try:
            with open("/content/drive/MyDrive/GAT/GAT_Embeddings/Emb_arxiv_lsh_add-False_linear.pickle", 'rb') as f:
                x = pickle.load(f)
            feat_data = x.detach().cpu().numpy()
            # feat_data = np.load('/content/drive/MyDrive/HonsProject/res/ogbn_arxiv/graphsage_20_20_500_1by2_final/allnodeembeddings.npy')
            print('loading GAT embeddings as node features ', feat_data.shape)
        except:
            print('failed to load emebeddings')
    if teleport_khop or augment_khop:
    ...
    data_loader_dic = {'feat_data': feat_data, 'labels': labels, 'adj_lists': adj_lists, 'train_mask': train_mask,
                       ...
                       'cluster_labels': [], 'freq': {}, 'dist_in_graph': {},
                       'centralityev': [], 'centralitybtw': [], 'centralityh': [],
                       'centralityd': [], 'lsh_neighbour_list': lsh_neighbourlist_dic, 'edge_index':edge_index}
    return data_loader_dic

##general feature space to graph
def create_adj_from_representation(feat_data, threshold=0.9, lsh_helper = None):
    lsh_neighbour_list = {}
    if lsh_helper == None:
        lsh_helper = {}
        lsh_helper['n_vectors'] = 16
        lsh_helper['num_lsh_neighbours'] = 100
        lsh_helper['atleast']=False
        lsh_helper['search_radius']=2
        print('Using default LSH settings ',lsh_helper)
    lsh_cand_dic = return_lsh_candidates(np.array(feat_data),n_vectors=lsh_helper['n_vectors'],
                                                 num_lsh_neighbours=lsh_helper['num_lsh_neighbours'],
                                                 atleast=lsh_helper['atleast'],
                                                 search_radius=lsh_helper['search_radius'])
    for node, neighbor_dic in lsh_cand_dic.items():
        temp = []
        for neigh,sim in neighbor_dic.items():
            if sim >= threshold:
                temp.append(neigh)
        lsh_neighbour_list[node] = set(temp)
    return lsh_neighbour_list
def create_graph_from_representation(path_file = '/content/drive/MyDrive/GNNOnImage/output_complete.pickle', threshold=0.9, lsh_helper = None, train_mask = None, test_mask = None, val_mask = None):
    with open(path_file,'rb') as f:
        data_dic = pk.load(f)
    print("loading representations from ",path_file)
    feat_data = data_dic['embeddings']
    try:
        labels = data_dic['labels'].detach().numpy()
    except:
        labels = np.array(data_dic['labels'])
    adj_lists = create_adj_from_representation(feat_data, threshold=threshold, lsh_helper=lsh_helper)
    num_nodes = len(adj_lists)
    if (train_mask == None) or (test_mask == None) or (val_mask == None):
        print('using random splits...')
        train_mask = []
        val_mask = []
        test_mask = []
        # np.random.seed(1)
        # rand_indices = np.random.permutation(num_nodes)
        rand_indices = np.arange(num_nodes)
        test = rand_indices[60000:]
        val = rand_indices[:10]
        train = list(rand_indices[10:60000])
        for node in range(num_nodes):
            if node in train:
                train_mask.append(True)
                val_mask.append(False)
    ...
    data_loader_dic = {'feat_data': feat_data, 'labels': labels, 'adj_lists': adj_lists, 'train_mask': train_mask,
                       ...
                       'cluster_labels': [], 'freq': {}, 'dist_in_graph': {},
                       'centralityev': [], 'centralitybtw': [], 'centralityh': [],
                       'centralityd': [], 'lsh_neighbour_list': []}
    return data_loader_dic