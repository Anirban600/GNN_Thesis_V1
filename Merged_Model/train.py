...
#################################################  Model First Phase  #########################################################
    ...
    # create model
    best_model = None
    for i in range(args.n_iter_stage_1):
        start = time.time()
        ...
        if not best_model: best_model = i, val_acc
        else: best_model = (i, val_acc) if best_model[1] < val_acc else best_model
        ...
        with open(f"Merged_Model/Embedding_dump_{i}.pickle", 'wb') as f: pickle.dump(data, f, protocol=pickle.HIGHEST_PROTOCOL)
        
    print(best_model)
    with open(f"Merged_Model/Embedding_dump_{best_model[0]}.pickle", 'rb') as f:
        data = pickle.load(f)
    out_emb = data['embeddings']

#####################################################   Create Augmented Graph   ####################################################################################
    
    for i in range(args.n_iter_stage_2):
...
if __name__ == '__main__':
    ...
    # Stage 1 Parameters
    parser.add_argument("--n_iter_stage_1", type=int, default=5, help="Number of Complete Iterations")
    ...
    # Stage 2 Parameters
    parser.add_argument("--n_iter_stage_2", type=int, default=5, help="Number of Complete Iterations")
