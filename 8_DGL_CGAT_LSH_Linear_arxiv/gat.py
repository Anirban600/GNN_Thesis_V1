...
def train(args, model, graph, feat_graph, labels, train_idx, val_idx, test_idx, optimizer, evaluator):
    ...
    optimizer.zero_grad()
    pred, _, Lg, Lb = model(graph, feat, feat_graph, labels)
    ...
    loss_act = custom_loss_function(pred[train_pred_idx], labels[train_pred_idx])
    loss = loss_act + Lamg * Lg + Lamb * Lb
    print(f"Actual loss : {loss_act.item()} Lg : {Lg.item()} Lb : {Lb.item()} Final Loss : {loss.item()}")
    loss.backward()
    optimizer.step()
...
def fake(args, model, graph, feat_graph, labels, train_idx, val_idx, test_idx, optimizer, evaluator):
    ...
    _, pred, Lg, Lb = model(graph, feat, feat_graph, labels)
...
def main():
    ...
    argparser.add_argument("--graph_margin", type=float, default=0.1, help="")
    argparser.add_argument("--class_margin", type=float, default=0.1, help="")
    argparser.add_argument("--lambda_g", type=float, default=0.1, help="")
    argparser.add_argument("--lambda_b", type=float, default=0.1, help="")
    argparser.add_argument("--top_k", type=int, default=3, help="")
