...
class TrueDropout(torch.nn.Module):
    ...
    def forward(self, x):
        if self.training:
            x = x.mul(torch.empty(x.size()[1], device=self.device).uniform_(0, 1) >= self.p) * (1 / (1 - self.p))
        return x.t()
...
class Encoder(nn.Module):
    ...
    def __init__(self, features, feature_dim, 
        ...
        self.weight = nn.Parameter(
                torch.empty(embed_dim, self.feat_dim if self.gcn else 2 * self.feat_dim, device=self.device).float())
        if dropout > 0:
    ...
    def forward(self, nodes):
        ...
        if not self.gcn:
            if self.cuda:
                ...
                self_feats = self.features(torch.tensor(nodes,device = self.device).long())
            else:
...
class LSHNeighboursEncoder(nn.Module):
    ...
    def __init__(self, features, feature_dim,
        ...
        if dropout > 0:
            ...
            self.dropout = None
        if self.lsh_augment:
            self.weight = nn.Parameter(torch.empty(embed_dim, self.feat_dim if self.gcn else 3 * self.feat_dim, device = self.device).float())
            if self.gcn:
                self.weight_lsh_gcn = nn.Parameter(torch.empty(embed_dim, self.feat_dim, device = self.device).float())
        else:
            self.weight = nn.Parameter(torch.empty(embed_dim, self.feat_dim if self.gcn else 2 * self.feat_dim, device=self.device).float())
        init.xavier_uniform(self.weight)
