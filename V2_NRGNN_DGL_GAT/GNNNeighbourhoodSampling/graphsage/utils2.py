...
def load_cora(lsh_helper, random_walk=False, root_folder='', emb_path="", teleport_khop=False, augment_khop=False, dfactor=2, use_centroid=False, teleport=0.2, planetoid = True, load_embeds = False):
    def give_freq(t2):
        frequency = {}
    ...
    if load_embeds:
        if not planetoid:
            ...
            with open('experiments/testing_xxx.pickle', 'rb') as f:
                feat_data = pickle.load(f)['embeddings'].cpu().detach().numpy()
            print('loading gs embeddings as node features ', feat_data.shape)
...
def load_wikics(lsh_helper, random_walk=False, type_walk='default', p=1, q=1, num_walks=10, walk_length=10,
    ...
    if load_embeds:
        feat_data = np.load('/content/drive/MyDrive/HonsProject/res/wikics/gswithembed/allnodeembeddings.npy')
        print('loading gs embeddings as node features ',feat_data.shape)
...
def load_ppi(lsh_helper, random_walk=False, root_folder='/content/drive/MyDrive/HonsProject/datasets/Dataset-PPI/ppi',
    ...
    if load_embeds:
        feat_data = np.load('/content/drive/MyDrive/HonsProject/res/ppi/graphsage_25_10_50/allnodeembeddings.npy')
        print('loading gs embeddings as node features ', feat_data.shape)
...
def load_ogbn_arxiv(lsh_helper,random_walk = False, root_folder='/content/drive/MyDrive/HonsProject/datasets',teleport_khop=False, augment_khop=False, dfactor=2, use_centroid=False, teleport=0.2, load_embeds = False):
...
def create_adj_from_representation(feat_data, threshold=0.7, lsh_helper = None):
    lsh_neighbour_list = {}
    if lsh_helper == None:
        lsh_helper = {}
        lsh_helper['n_vectors'] = 16
        lsh_helper['num_lsh_neighbours'] = 3000
        lsh_helper['atleast']=False
        lsh_helper['search_radius']=1
...
def create_graph_from_representation(path_file = '/content/drive/MyDrive/GNNOnImage/output.pickle', threshold=0.7, lsh_helper = None, train_mask = None, test_mask = None, val_mask = None):
    ...
    if (train_mask == None) or (test_mask == None) or (val_mask == None):
        ...
        train = list(rand_indices[int(0.5 * num_nodes):])
        for node in range(num_nodes):
            if node in train:
                train_mask.append(True)
                val_mask.append(False)
