...
parser.add_argument('--edge_hidden', type=int, default=64,help='Number of hidden units of MLP graph constructor')
parser.add_argument('--dropout', type=float, default=0.5,help='Dropout rate (1 - keep probability).')
parser.add_argument('--dataset', type=str, default="cora", choices=['cora', 'citeseer','pubmed','dblp'], help='dataset')
parser.add_argument('--ptb_rate', type=float, default=0.2, help="noise ptb_rate")
parser.add_argument('--epochs', type=int,  default=500, help='Number of epochs to train.')
parser.add_argument('--lr', type=float, default=0.001, help='Initial learning rate.')
parser.add_argument('--alpha', type=float, default=0.03, help='weight of loss of edge predictor')
parser.add_argument('--beta', type=float, default=1, help='weight of the loss on pseudo labels')
parser.add_argument('--t_small',type=float, default=0.1, help='threshold of eliminating the edges')
parser.add_argument('--p_u',type=float, default=0.8, help='threshold of adding pseudo labels')
parser.add_argument("--n_p", type=int, default=50, help='number of positive pairs per node')
...
# else:
#     data = Dataset(root='/content/drive/MyDrive/V2_NRGNN_DGL_GAT/NRGNN/data', name=args.dataset)
...
features = out['feat_data']
labels = out['labels']
nodes = np.arange(labels.shape[0])

idx_train = nodes[out['train_mask']]
idx_val = nodes[out['val_mask']]
idx_test = nodes[out['test_mask']]

adj_lists = out['adj_lists']
row, col = [], []

for n in adj_lists:
    row += [n] * len(adj_lists[n])
    col += list(adj_lists[n])

data = [1] * len(row)
adj = csr_matrix((data, (row, col)))

with open(args.label_path, 'rb') as f:
    noise_labels = pickle.load(f)
